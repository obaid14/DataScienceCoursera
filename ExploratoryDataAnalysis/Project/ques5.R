...
vehdata <- pmdata[pmdata$fips == 24510 & pmdata$type == "ON-ROAD",]

# Making the year column as a factor
vehdata$year <- as.factor(vehdata$year)

# Finding the sum of emissions for each year
sum_data <- tapply(vehdata$Emissions,vehdata$year,FUN=sum)

# Setting emissions in terms of thousand tons
sum_data <- sum_data / 1000
...
barplot(sum_data, yaxt = "n", main = "PM2.5 Emissions due to Motor Vehicles in Baltimore (1999-2008)")
...
# Copying the plotted graph to a file names plot5.png as a png file
